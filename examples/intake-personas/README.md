...
Each has a config; run the deterministic intake against any of them:
...
The collaborative halves of levels 2-3 are exercised by the `chain-intake` agent
(see `.claude/agents/chain-intake.md` and `canon/intake-interview-guide.md`); the
deterministic classification, maturity, plan, and manifest behavior for all three is
covered by `tests/test_intake.py`.
