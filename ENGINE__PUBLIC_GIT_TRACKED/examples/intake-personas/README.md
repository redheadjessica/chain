...
root (`./chain` keeps your working directory there — paths below are relative to it):

    ./chain intake ENGINE__PUBLIC_GIT_TRACKED/examples/intake-personas/p1-organized.config.yaml
...
| `p2-studio` (Sweet Fern Studio, reuses `ENGINE__PUBLIC_GIT_TRACKED/examples/demo-sources-studio/`) | 2 | scattered FAQs/reviews/notes classify into a source map; missing guidance is distilled FROM the material |
