"""Deterministic changelog structure logic for CHAIN's changelog synthesis.

Pure functions only — no Git, no network, no filesystem. `changelog_sync.py` is the
runner that wires these into a live pass. Kept separate (stdlib-only, like
`path_safety.py` and `editorial_library.py`) so parsing/sorting/marker rules are
testable with zero installs.
"""
...
def meaningful_changed_files(files: list) -> list:
    ...
    changelog, the drift report, or the synthesis module/tests) never counts as
    meaningful work, so it can never trigger itself. When synthesis code changed as
    part of a bigger commit, pyproject.toml is also treated as synthesis maintenance
